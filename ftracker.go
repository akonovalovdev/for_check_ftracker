...
func (t Training) meanSpeed() float64 {
	...
	return distance / float64(t.Duration)
}
...
func (r Running) Calories() float64 {
	...
	spentCal = (CaloriesMeanSpeedMultiplier * r.meanSpeed() * CaloriesMeanSpeedShift) * r.Weight / MInKm * float64(r.Duration) * MinInHours
	return spentCal
}
...
func (r Running) TrainingInfo() InfoMessage {
	// Для реализации этого метода для структуры Running{} нужно вернуть вызов такого же метода TrainingInfo(),
	// только для структуры Training{}. Вспомните, как вызывать методы встроенной структуры
...
func (s Swimming) Calories() float64 {
	var spentCal float64
	spentCal = (s.meanSpeed() + SwimmingCaloriesMeanSpeedShift) * SwimmingCaloriesWeightMultiplier * s.Weight * float64(s.Duration)
	return spentCal
}
